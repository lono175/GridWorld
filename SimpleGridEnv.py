...
class SimpleGrid:
    def __init__(self, size, imgSize, actionList, monsterMoveProb):
        self.size = self.height, self.width = size
    ...
    def start(self, numOfTurtle, numOfCoin):
        ...
        #add mario
        self.marioLoc = self.getNewLoc(locList)
        self.world[self.marioLoc] = 1
    ...
    def getScreen(self):
        ...
        for y in range(0, self.imgSize[1], incY):
            pygame.draw.line(self.screen, green, (0, y), (self.imgSize[0], y), 2);
        ...
        for y in range(0, self.width):
            for x in range(0, self.height):
                if self.world[(x, y)] == 1:
                    ...
                    rect.center = (offsetX + y*incX, offsetY + x*incY)
